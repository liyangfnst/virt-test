...
def run_virt_sysprep(test, params, env):
    ...
    def prepare_action(vm):
        ...
        try:
            ...
            tmp_hostname = "%s_tmp" % sysprep_hostname
    ...
    def modify_network(vm_name, nic_dict):
        """
        Modify domain's interface to make sure domain can be login.
        """
        iface_type = nic_dict['iface_type']
        iface_source = nic_dict['iface_source']
        mac_address = nic_dict['mac_address']
        try:
            ...
            raise error.TestError("Modify network failed:%s" % detail)
    ...
    def clean_clone_vm():
        ...
        try:
            ...
            raise error.TestNAError("Clean clone guest failed!:%s" % detail)
    ...
    if len(disks):
        ...
        raise error.TestNAError("Can not get disk of %s" % vm_name)

    # Do some prepare action
    vm_clone_name = "%s_clone" % vm_name
    clone_image = "%s_clone.img" % image
    nic_dict = vm_xml.VMXML.get_first_nic(vm_name)
    clean_clone_vm()
    ...
    if clone_result.exit_status:
        raise error.TestError("virt-clone failed.")
    try:
        # Modify network to make sure the clone guest can be logging.
        modify_network(vm_clone_name, nic_dict)
        ...
        if sysprep_type == "resize":
            img_size = os.path.getsize(clone_image)/1024/1024/1024
            resize_image = "%s_resize.img" % clone_image
            utils.run("qemu-img create -f raw %s %dG" % (resize_image,
                                                         (img_size+1)))
            lgf.virt_resize_cmd(clone_image, resize_image)
